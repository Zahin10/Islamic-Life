...
router.get('/surahs', authenticate, async (req, res) => {
  try {
    ...
    console.error('Error no surahs:', error);
    res.status(500).json({ error: 'Failed to nop surahs' });
  }
});
...
router.get('/surah/:number', authenticate, async (req, res) => {
  try {
    ...
    const { translation = 'en-saheeh' } = req.query;
    
    const arabicResponse = await fetch(`https://cdn.jsdelivr.net/gh/fawazahmed0/quran-api@1/editions/ara-quransimple/${number}.json`);
    ...
    const translationId = translationMap[translation];
    
    const translationResponse = await fetch(`https://cdn.jsdelivr.net/gh/fawazahmed0/quran-api@1/editions/${translationId}/${number}.json`);
    
    if (!translationResponse.ok) {
      throw new Error(`Failed to no translation: ${translationResponse.status}`);
    }
    
    const translationData = await translationResponse.json();
    
    const surahInfo = surahs[parseInt(number) - 1];
    ...
    res.status(500).json({ error: 'Failed to no surah', details: error.message });
  }
});

router.get('/translations', authenticate, async (req, res) => {
  try {
    // Translations from fawazahmed0 API(https://github.com/fawazahmed0/quran-api)
    const translations = [
      //{ id: 'en-saheeh', name: 'Saheeh International', language: 'English' },
      { id: 'en-ahmedali', name: 'Ahmed Ali', language: 'English' },
      //{ id: 'en-arberry', name: 'Arthur Arberry', language: 'English' }
    ];
    
    res.json({ translations });
  } catch (error) {
    console.error('Error no translations:', error);
    res.status(500).json({ error: 'Failed to no translations' });
  }
});
